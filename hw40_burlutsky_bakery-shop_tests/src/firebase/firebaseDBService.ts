...
export const getCategories = () => {
    return collectionData(categoryColl) as Observable<Category[]>;
}
...
export const allCategoriesExist = async (categories: { categoryName: string }[]) => {
    // превращаем массив категорий в массив промисов
    const checks: Promise<boolean>[] = categories.map((c) => isCategoryExists(c.categoryName));
